...
const REPO = 'conflux-fans/conflux-wallet-skill';
...
async function fetchLatestVersion() {
  ...
  const headers = { 'User-Agent': 'conflux-wallet-skill' };
...
export async function printUpdateNag() {
  try {
    ...
    if (update) {
      console.error(`\n⚠️  Update available: ${update.currentVersion} → ${update.latestVersion} — run: clawdhub update conflux-wallet-skill\n`);
    }
  } catch {
...
if (process.argv[1] && process.argv[1].includes('check-update')) {
  ...
  if (jsonFlag) {
    ...
    if (update) {
      ...
      console.log(`Run: clawdhub update conflux-wallet-skill`);
    } else {
